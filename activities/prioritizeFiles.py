...
from utils.dataclasses import ChangeSet, ChangedFile
...
def compute_risk_score(file: ChangedFile) -> float:
    """
    ...
    Returns:
        Risk score as a float (higher = more important/risky)
    """
    score = 0.0
...
async def prioritize_files(change_set: dict) -> List[dict]:
    """
    ...
    Returns:
        List of file dictionaries sorted by risk score (highest first),
        each containing the file data with an added 'risk_score' field
    """
    ...
    for file_data in files_data:
        ...
        # Compute risk score
        risk_score = compute_risk_score(file_for_scoring)
        
        # Create result entry
        if isinstance(file_data, dict):
            result_file = file_data.copy()
        else:
            # Convert ChangedFile to dict
            result_file = {
                'path': file_path,
                'added': file_added,
                'removed': file_removed,
        ...
        result_file['risk_score'] = risk_score
        prioritized_files.append(result_file)
